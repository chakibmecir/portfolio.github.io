const s = (p) => {
  ...
  p.preload = () => {
    audio = p.loadSound("audio/audio-remix.mp3");
    demo8Shader = p.loadShader("shaders/base.vert", "shaders/d8.frag");
    img = p.loadImage("img/9-min-compressed.jpg");
    d_map = p.loadImage("img/ripple.jpg");
  };
  ...
  p.draw = () => {
    ...
    const mapBass = p.map(bass, 0, 255, 5, 8.0);
    const mapTremble = p.map(treble, 0, 150, 0, 0.0);
    const mapMid = p.map(mid, 0, 255, 0.0, 0.1);

    demo8Shader.setUniform("u_time", p.frameCount / 40);
